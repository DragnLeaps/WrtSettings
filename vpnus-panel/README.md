...
Роутер видит домены, а не процессы Windows. Поэтому приложения, использующие собственный DNS/DoH, QUIC с нестандартным DNS или общие CDN, могут потребовать дополнительные домены. Все ручные и быстрые доменные исключения автоматически добавляются в IPv4- и IPv6-наборы при DNS-запросах клиента. Для пресета Telegram дополнительно исключаются его серверные IPv4- и IPv6-сети, которые мобильное приложение может использовать без DNS-запроса к `telegram.org`.
