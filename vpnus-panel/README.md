...
Роутер видит домены, а не процессы Windows. Поэтому приложения, использующие собственный DNS/DoH, QUIC с нестандартным DNS или общие CDN, могут потребовать дополнительные домены. Список IP обновляется раз в 5 минут. Для пресета Telegram дополнительно исключаются его серверные IPv4-сети, которые мобильное приложение может использовать без DNS-запроса к `telegram.org`.
